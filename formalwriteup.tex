...
The rational Cherednik algebra $\HH_{\hbar, c}(\h)$ admits a $\ZZ_{\geq 0}$-graded representation on the polynomial ring $A = \Sym(\h^*)$, known as the \textit{polynomial representation}. In particular, for any $f \in A$ we can take a partial derivative $\frac{\partial f}{\partial x_i}$ by noting that $A=k[x_0,\dots,x_{n-2}]$ with $x_{n-1}=-x_0-\dots-x_{n-2}$; then we can write $f$ as a polynomial and take its partial derivative by any $x_i$ with $0 \le i \le n-2$. 

The actions of $\Sym(\h^*)$ and $k[S_n]$ on $A$ are by left multiplication and the $S_n$ action on $\h^*$, respectively.  The action of $\Sym(\h)$ is implemented by letting $y \in \h$ act via the \textit{Dunkl operator} $D_y$ defined by
\begin{align*}
...
\[
D_i = \hbar\frac{\partial f}{\partial x_i} -c \sum_{k \ne i} \frac{(1-s_{ki})f}{x_i-x_k}
\]
...
\todo[inline]{Break up the proof into lemmas, e.g. for the value of $\frac{\partial F}{\partial z}$}
...
Then it remains only to show that $[z^l]G_i(z)=0$ for $l=0,\dots,p$. To show this, we need only show that $G_i(z)=z^{p+1}g_i(z)$ for some power series $v(z)$. 

We consider the expansion of $g(z)$; this is $g(z)=\prod_{j=0}^{n-1} (1-x_jz) = 1-z\left(\sum_i x_i\right)+z^2e_2(x_i)+\dots+z^ne_n(x_i)$ where $e_2,\dots,e_n$ are polynomials in the $x_i$. In particular we note that $x_{n-1}=-x_0-\dots-x_{n-2}$ by hypothesis, so $z^2 \mid g(z)-1$. 

Then by the definition of $G_i(z)$, we see that $z(g-1)^{p-1} \mid G_i(z)$, so $z^{2p-1} \mid V(z)$. Since $p \ge 2$, it is clear that $z^{p+1} \mid G_i(z)$, so we can write $G_i(z)=z^{p+1}g_i(z)$ for some power series $g_i(z)$ as desired. 
...
\todo[inline]{Split computations out into some lemmas on computations of $F'(z)$ and $V(z)$ which explicitly mention the hypotheses that $\sum_i m_i y_i = 0$.}

To show that $A/I$ is a complete intersection, we will show that if $x_0,\dots,x_{n-1} \in k$ satisfy $f_i(x_0,\dots,x_{n-1})=0$ for all $i$, then $x_0=\dots=x_{n-1}=0$. We will use a `residues by parts' argument; using the fact that the residue of an exact differential is zero, we will relate the coefficients of a power series to the coefficients of the derivative of that power series and derive a contradiction. To show this we must consider the derivatives of some power series in $z$ (including $F(z)$) with coefficients from $k$ rather than coefficients from $A$.


\begin{lemma}\label{lem:dFdz2}
For any $y_0,\dots,y_{r-1}$ distinct elements of $k$ with $m_i$ positive integers such that $\sum_i m_iy_i=0$, if we let $g(z)=\prod_{i=0}^{r-1} (1-y_iz)^{m_i}$ and $F(z)=\sum_{k=0}^{p-1} \binom{c}{k}(g-1)^k$, then
\[
...
\end{lemma}
\begin{proof}
The computation proceeds essentially as it did for Lemma~\ref{lem:dFdz}.
...
%&=\left(\sum_j \frac{-x_j}{1-x_jz}\right)\left(\sum_{k=1}^{p-1}k\binom{c}{k}(g-1)^{k-1}\right)g(z)\\
%&=\left(\sum_j \frac{-x_j}{1-x_jz}\right)\left(\sum_{k=0}^{p-2}(k+1)\binom{c}{k+1}(g-1)^{k}\right)g(z)\\
&=\left(\sum_j\frac{-m_jy_j}{1-y_jz}\right)\left(\sum_{k=0}^{p-2}c\binom{c-1}{k}(g-1)^{k}\right)(g-1+1)\\
%&=\left(\sum_j \frac{-x_j}{1-x_jz}\right)\left(\left(\sum_{k=0}^{p-2}c\binom{c-1}{k}(g-1)^{k}\right)+\left(\sum_{k=0}^{p-2}c\binom{c-1}{k}(g-1)^{k+1}\right)\right)\\
&=\left(\sum_j\frac{-m_jy_j}{1-y_jz}\right)\left(\left(\sum_{k=0}^{p-2}c\binom{c-1}{k}(g-1)^{k}\right)+\left(\sum_{k=1}^{p-1}c\binom{c-1}{k-1}(g-1)^{k}\right)\right)\\
&=\left(\sum_j \frac{-m_jy_j}{1-y_jz}\right)\left(-c\binom{c-1}{p-1}(g-1)^{p-1}+\sum_{k=0}^{p-1}c\binom{c}{k}(g-1)^{k}\right)\\
&=\left(\sum_j \frac{-cm_jy_j}{1-y_jz}\right)F(z)+\left(\sum_j \frac{-m_jy_j}{1-y_jz}\right)\left(-c\binom{c-1}{p-1}(g-1)^{p-1}\right).
\end{align*}
We define 
\[
V(z)=\left(\sum_j \frac{-m_jy_j}{1-y_jz}\right)\left(-c\binom{c-1}{p-1}(g-1)^{p-1}\right).
\]
It is then clear that 
\[
F'(z)=V(z)+\left(\sum_j \frac{-cm_jy_j}{1-y_jz}F(z)\right).
...
We consider the expansion of $g(z)$; this is $g(z)=\prod_j (1-y_jz)^{m_j} = 1-z\left(\sum_i m_iy_i \right)+z^2e_2(y_i)+\dots+z^ne_n(y_i)$ where $e_2,\dots,e_n$ are polynomials in the $y_i$. In particular we note that $\sum_i m_iy_i=0$ by hypothesis, so $z^2 \mid g(z)-1$. 

Then by the definition of $V(z)$, we see that $(g-1)^{p-1} \mid V(z)$, so $z^{2p-2} \mid V(z)$. Since $p \ge 2$, it is clear that $z^p \mid V(z)$, so we can write $V(z)=z^pv(z)$ for some power series $v(z)$ as desired. 
\end{proof}

\begin{lemma} \label{lem:resbyparts}
For any $y_0,\dots,y_{r-1}$ distinct elements of $k$ such that $\prod_i y_i \ne 0$ and $m_i$ positive integers such that $\sum_i m_iy_i=0$, define $g(z), F(z)$ as in Lemma~\ref{lem:dFdz2}. Let 
\[
a(z)=\frac{1}{\prod_{i=0}^{r-1}(1-y_iz)}F(z).
\].
Then we have
\begin{enumerate}
\item $a'(z)=z^pb(z)-\sum_j \frac{(m_jc-1)y_j}{1-y_jz}a(z)$ for some power series $b(z)$;
\item For any integer $l \le p-r$, $[z^l]a(z)$ is a linear combination of $[z^{l+1}]a(z),\dots,[z^{l+r}]a(z)$.\end{enumerate}

\end{lemma}

\begin{proof} We begin by computing $a'(z)$. By Lemma~\ref{lem:dFdz2} we have
\[
F'(z)=V(z)+\left(\sum_j \frac{-cm_jy_j}{1-y_jz}F(z)\right),
\]
where $[z^l]V(z)=0$ for $0 \le l \le p-1$. 
...
\end{align*}
Since $[z^l]V(z)=0$ for $0 \le l \le p-1$ we see that $\frac{V(z)}{\prod_{i=0}^{r-1}(1-y_iz)}=z^pb(z)$ for some power series $b(z)$. Therefore
\[
...
We will assume that $\prod_i y_i \ne 0$ in order to derive a contradiction and show that one of the $y_i$ is $0$. 
Let
\[
a(z)=\frac{1}{\prod_{i=0}^{r-1}(1-y_iz)}F(z)
\]
as in Lemma~\ref{lem:resbyparts}.
...
\]
Choosing these $\lambda_i^k$ in (\ref{eq:lincomb}), we see that $[z^{p-r+1}]a(z)=\dots=[z^p]a(z)=0$. Then by repeated application of Lemma~\ref{lem:resbyparts} starting with $l=p-r$, we find that $[z^l]a(z)=0$ for $l=p-r,p-r-1,\dots,0$. Therefore, $[z^0]a(z)=0$; a contradiction. We conclude that $y_i=0$ for some $i$.

Assume without loss of generality that $y_{r-1}=0$. If $r>1$, then we note that since the $y_i$ were distinct that $\prod_{i=0}^{r-2} y_i \ne 0$ and $\sum_{i=0}^{r-1} m_iy_i = \sum_{i=0}^{r-2} m_iy_i=0$; therefore we are still able to apply Lemma~\ref{lem:resbyparts} to $y_0,\dots,y_{r-2}$. By redefining the $b_i(z)$ appropriately and repeating the argument above, we find that one of the $y_i$ with $i < r-1$ is $0$, which gives a contradiction since the $y_i$ are distinct. Therefore we must have $r=1$ and $y_0=0$. 
\end{proof}
