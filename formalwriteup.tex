...
\[
D_{y_i - y_j} = \hbar \frac{\partial}{\partial (y_i-y_j)}-c\left(\sum_{k \ne i} \frac{1-s_{ki}}{x_i-x_k}\right)-c\left(\sum_{k \ne j} \frac{1-s_{ki}}{x_j-x_k}\right)
\]

where $\frac{\partial}{\partial (y_i-y_j)}$ is a differential operator we compute by letting $\frac{\partial x}{\partial (y_i-y_j)} = \langle y_i-y_j,x\rangle$ for all $x \in \h^*$ and extending via the Leibnitz rule to the rest of $A=\Sym(\h^*)$. 
...
\[
g(z)=\prod_{j=0}^{n-1} (1-x_jz) = 1-z\sum_i x_i+z^2e_2(x_0, \ldots, x_{n - 1})+\dots+(-1)^nz^ne_n(x_0, \ldots, x_{n - 1})
\]
...
F'(z) = V(z) - \sum_{j=0}^{n-1} \frac{cx_j}{1-x_jz} F(z).
\]
It remains only to show that $[z^l]V(z)=0$ for $l=0,\dots,p-1$.  By definition of $V(z)$, we see that $(g-1)^{p-1} \mid V(z)$, hence by Lemma~\ref{lem:z2g}, $z^{2p-2} \mid V(z)$. Since $p \ge 2$, this implies $z^p \mid V(z)$, completing the proof.
\end{proof}

\begin{lemma}\label{lem:dFdxi} We have
\[
\frac{\partial F}{\partial(y_1-y_0)} = G(z) - \left(\frac{zc}{1-x_1z}-\frac{zc}{1-x_0z}\right)F(z)
\]
...
We see that
\[
\frac{\partial g}{\partial(y_1-y_0)}=-\frac{zg}{1-x_1z}+\frac{zg}{1-x_0z}.
\]
Then we see that
\begin{align*}
\frac{\partial F}{\partial(y_1-y_0)}&=\left(\sum_{k=1}^{p-1}k\binom{c}{k}(g-1)^{k-1}\right)\frac{\partial g}{\partial (y_1-y_0)}\\
&=\left(-\frac{z}{1-x_1z}+\frac{z}{1-x_0z}\right)\left(\sum_{k=1}^{p-1}k\binom{c}{k}(g-1)^{k-1}\right)g\\
&=\left(-\frac{z}{1-x_1z}+\frac{z}{1-x_0z}\right)\left(\sum_{k=0}^{p-2}c\binom{c-1}{k}(g-1)^{k}+\sum_{k=0}^{p-2}c\binom{c-1}{k}(g-1)^{k+1}\right)\\
&=\left(-\frac{zc}{1-x_1z}+\frac{zc}{1-x_0z}\right)\left(-\binom{c-1}{p-1}(g-1)^{p-1}+F(z)\right).
\end{align*}
We define
\[
G(z)=\left(-\frac{zc}{1-x_1z}+\frac{zc}{1-x_0z}\right)\left(-\binom{c-1}{p-1}(g-1)^{p-1}\right).
\]
Then we see that
\[
\frac{\partial F}{\partial(y_1-y_0)}= G(z) - \left(\frac{zc}{1-x_1z}-\frac{zc}{1-x_0z}\right)F(z)
\]
It remains only to show that $[z^l]G(z)=0$ for $l=0,\dots,p$. By definition of $G(z)$, we see that $z(g-1)^{p-1} \mid G(z)$, so by Lemma~\ref{lem:z2g}, $z^{2p-1} \mid G(z)$. Since $p \ge 2$, this implies $z^{p+1} \mid G(z)$, completing the proof.
\end{proof}
...
By Lemma~\ref{lem:dFdxi} we recall that $\frac{\partial F}{\partial(y_1-y_0)}= G(z) - \left(\frac{zc}{1-x_1z}-\frac{zc}{1-x_0z}\right)F(z)$ for a power series $G(z)$ with $[z^l]G(z)=0$ for $l=0,\dots,p$. 

We can now calculate $\frac{\partial F_0}{\partial (y_1-y_0)}$. We compute
\begin{align*}
\frac{\partial F_0}{\partial (y_1-y_0)}
&=-\frac{z}{(1-x_0z)^2}F(z)+\frac{1}{1-x_0z}\frac{\partial F}{\partial (y_1-y_0)}\\
&=-\frac{z}{1-x_0z}F_0(z)+\frac{1}{1-x_0z}\left(\frac{zc}{1-x_0z}-\frac{zc}{1-x_1z}\right)F(z)+\frac{G(z)}{1-x_0z}\\
&=\left(\frac{z(c-1)}{1-x_0z}-\frac{zc}{1-x_1z}\right)F_0(z)+\frac{G(z)}{1-x_0z}.
\end{align*}
...
&=\frac{z}{1-x_iz}F_0(z).
\end{align*}
We recall that we need only consider the action of $D_1-D_0$ on $F_0(z)$. We see that 
\[
D_1-D_0=\frac{\partial}{\partial (y_1-y_0)}-c\left(\sum_{j > 0} \frac{1-s_{0j}}{x_0-x_j}\right)-c\left(\frac{1-s_{01}}{x_1-x_0}\right)
\]
since $F_0$ is invariant under $s_{ij}$ where $0 < i,j$. We now compute
\begin{align*}
(D_1-D_0)F_0&=\left(\frac{\partial}{\partial (y_1-y_0)}-c\left(\sum_{j > 0} \frac{1-s_{0j}}{x_0-x_j}\right)-c\left(\frac{1-s_{01}}{x_1-x_0}\right)\right)(F_0)\\
&=\frac{\partial F_0}{\partial (y_1-y_0)}-c\sum_{j > 1} \left(\frac{1-s_{0j}}{x_0-x_j}\right)(F_0)\\
&=\frac{G(z)}{1-x_0z}+\left(\frac{z(c-1)}{1-x_0z}-\frac{zc}{1-x_1z}\right)F_0(z)-\sum_{j > 1} \frac{zc}{1-x_jz}F_0(z)\\
&=\frac{G}{1-x_0z}+\frac{z(c-1)}{1-x_0z}F_0+\sum_{j>0} \frac{zc}{1-x_jz}F_0.
\end{align*}
...
\begin{align*}
(D_1-D_0)(F_0)&=\frac{G(z)}{1-x_0z}+\frac{z(c-1)}{1-x_0z}F_0+\sum_{j>0} \frac{zc}{1-x_jz}F_0\\
&=\frac{G(z)}{1-x_0z}-zF_0+zF_0-\frac{z}{1-x_0z}F_0+\left(\sum_{j} -zcF_0+\frac{zc}{1-x_jz}F_0\right)\\
&=\frac{G(z)}{1-x_0z}-zF_0+\frac{-x_0z^2}{1-x_0z}F_0+\left(\sum_{j} \frac{x_jcz^2}{1-x_jz}F_0\right)\\&=\frac{G(z)}{1-x_0z}-zF_0-z^2\frac{\partial F_0}{\partial z}+z^2\frac{V(z)}{1-x_0z}\\
&=\frac{G(z)+z^2V(z)}{1-x_0z}-zF_0-z^2\frac{\partial F_0}{\partial z}.
\end{align*}

We note that $[z^p]\left(\frac{G+z^2V}{1-x_0z}\right)$ is a linear combination of $[z^l](G(z)+z^2V(z))$ for $0 \le l \le p$. This means that $[z^p]\left(\frac{G+z^2V}{1-x_0z}\right)$ is a linear combination of $[z^l]G(z)$ for $0 \le l \le p$ and $[z^l]V(z)$ for $0 \le l \le p-2$. By Lemmas~\ref{lem:dFdz} and \ref{lem:dFdxi} we recall that the coefficients of $G(z),V(z)$ under consideration are all $0$. This implies that $[z^p]\left(\frac{G+z^2V}{1-x_0z}\right)=0$. Therefore
\[
