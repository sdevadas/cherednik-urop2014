...
Given the symmetric group $S_n$, we see that $V=k^n$ is a representation of $S_n$ in a natural way (permutation of coordinates). Let $s_{ij}$ be the transposition permuting indices $i$ and $j$. We let $y_0,\dots,y_{n-1}$ be a basis of $V$; we define an $(n-1)$-dimensional representation $\h$ of $S_n$ as the span of $y_i-y_j$ for $i \ne j$. We choose a dual basis $x_0,\dots,x_{n-1}$ for $V^*$; the dual representation $\h^*$ of $\h$ can be realized as the quotient of $V^*$ by $x_0+\dots+x_{n-1}$. $S_n$ acts on $\h,\h^*$ by permuting the basis vectors in the natural way. 
...
\begin{align*}
[x_i,x_j]=0, \quad [y_i,y_j] = 0, \quad [y_i,x_j] = \hbar\langle y_i,x_j\rangle - \sum_{k=0}^{n-1}\sum_{\ell=0}^{n-1} c\langle y_i,x_k-x_\ell \rangle\langle y_k-y_\ell,x_j\rangle s_{k\ell}
\end{align*}
...
A statement holds for $c$ {\it generic} if it holds away from countably many values for $c$. Suppose that $\hbar=1$, $c$ generic, and the characteristic $p$ of $k$ dividing $n$.
 
 \newtheorem*{thm:main}{Theorem \ref{thm:main}}
\begin{thm:main} The irreducible quotient $A/J$ of the polynomial representation for $\HH_{\hbar,c}(\h)$ is a complete intersection with Hilbert series $\left(\frac{1-t^p}{1-t}\right)^{n-1}$. The maximal ideal $J$ is has linearly independent generators $f_i=[z^p]F_i(z)$ for $0 \le i \le n-2$ with
\[
...
\subsection{PBW decomposition}


We can give $\HH_{\hbar,c}(\h)$ a $\mathbb{Z}$-grading by setting $\deg x=1$ for $x \in \h^*$, $\deg y = -1$ for $y \in \h$, and $\deg g=0$ for $g \in k[G]$. We get the PBW-type decomposition $\HH_{\hbar,c}(\h)=\Sym(\h) \otimes_k k[S_n] \otimes_k \Sym(\h^*)$ (\cite[Section 3.2]{EM}). 
...
\end{align*}
The quotient $A/J$ is a finite-dimensional irreducible $\mathbb{Z}$-graded representation of $\HH_{\hbar,c}(\h)$. The {\it Hilbert series} $h_{A/J}(t)$ of $A/J$ is $\sum_{j=0}^\infty (\dim_k L_j)t^k$ where $L_j$ is the degree $j$ subspace of $L$. 
...
\begin{align*}
D_y(f) = \hbar \partial_y f  - \sum_{k=0}^{n-1} \sum_{\ell=0}^{n-1} c \frac{ \langle y, x_k-x_\ell \rangle}{x_k-x_\ell} (1-s_{k\ell}). f.
\end{align*}

 If $D_i$ is the Dunkl operator corresponding to $y_i$, the Dunkl operators for the elements of $\h$ are spanned by $D_i-D_j$ with $i \ne j$. 
...
\begin{theorem}\label{thm:main} The irreducible quotient $A/J$ of the polynomial representation for $\HH_{\hbar,c}(\h)$ is a complete intersection with Hilbert series $\left(\frac{1-t^p}{1-t}\right)^{n-1}$. The maximal ideal $J$ is has linearly independent generators $f_i=[z^p]F_i(z)$ for $0 \le i \le n-2$ with
\[
F_i(z)=\frac{1}{1-x_iz} \left( \sum_{k=0}^{p-1} \binom{c}{k}(-1+\prod_{j=0}^{n-1} (1-x_jz))^k\right) \text{ for $i=0,\dots,n-2$}.
\]
\end{theorem}
