...
\]
\end{proposition}
We interpret the characteristic $p$ analogue of Proposition \ref{prop:ce} to mean that if $r = p$ and $p \mid n$, then since $p/n$ is an indeterminacy in characteristic $p$ (because it is equivalent to $0/0$), $c=p/n$ in characteristic $0$ in some sense corresponds to $c$ generic in characteristic $p$.

While this substitution is of course invalid, Proposition \ref{prop:ce} may be interpreted as a statement about certain formal power series.  By using a power series version of this construction of generators which makes sense in characteristic $p$, we are able to mimic the arguments of \cite{BEG, CE} to show that they cut out a complete intersection and generate the entire ideal.  We believe that the philosophy of taking characteristic $p$ analogues of characteristic $0$ results for the rational Cherednik algebra should apply more generally and hope to explore this further in future work.
...
\end{corollary}
\begin{proof}
Apply Lemma~\ref{lem:dFdz} by letting the $x_i$ be $m_i$ copies of the $s_i$. 
\end{proof}
...
\]
By Proposition~\ref{prop:ann}, the generators $f_i$ of $I$ are singular vectors, so $I \subseteq J_c$, implying that $h_{A/I}(t) \ge h_{A/J_c}(t)$ coefficient-wise.  We conclude that $h(t) = 1$, hence $h_{A/I}(t)=h_{A/J_c}(t)$ and $I=J_c$, completing the proof.
\end{proof}
