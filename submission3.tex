...
\end{proposition}
\begin{proof}
It suffices to show that if $\alpha_1,\dots,\alpha_{n} \in k$ satisfy $f_i(\alpha_1,\dots,\alpha_{n})=0$ for all $i$, then $\alpha_1=\dots=\alpha_{n}=0$.  Suppose that the $\alpha_i$ take the distinct values $\{s_1,\dots,s_{r}\}$, where $s_i$ occurs with multiplicity $m_i>0$ so that $g(z)=\prod_i (1-s_iz)^{m_i}$.  In these terms, we have that 
\[
