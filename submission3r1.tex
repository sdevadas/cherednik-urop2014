...
Let $s_1,\dots,s_{r}$ be distinct non-zero elements of $k$ and $m_i$ positive integers so that that $\sum_i m_is_i=0$.  For generic $c$ and any non-negative integer $l \le p-r$, $[z^l]a(z)$ is a linear combination of $[z^{l+1}]a(z)$, $\ldots$, $[z^{l+r}]a(z)$.
\end{lemma}
\begin{proof} 
...
\]
so for $p-l-r \ge 0$, we see that $[z^{-1}]\left(\frac{d}{dz}( h_l(z) a(z))\right)=0$ is a linear combination of $[z^l]a(z)$, $\ldots$, $[z^{l+r}]a(z)$ in which $[z^l]a(z)$ has coefficient $(-1)^r \prod_i s_i\left((l + r) - \sum_i m_i c\right)$. This coefficient is nonzero since $\prod_i s_i \ne 0$ and $c \ne \frac{l+r}{\sum_i m_i}$ for generic $c$.

\todo[inline]{This does not work if $\sum_i m_i = 0$ and $l + r = 0$, which is where we use it.}

 Therefore $[z^l]a(z)$ is a linear combination of $[z^{l+1}]a(z)$, $\dots$, $[z^{l+r}]a(z)$ when $l \le p-r$. 
\end{proof}

\begin{remark}
In Lemma \ref{lem:resbyparts}, $c$ must be generic in the sense that it must avoid finitely many values for each choice of $(l, r, \{m_i\})$.
\end{remark}
...
\end{proposition}
\begin{proof}
By Proposition~\ref{prop:linind}, $I_c$ has a set of $n-1$ linearly independent and therefore minimal generators $\{f_i\}$ in degree $p$.  We will now show that if $\alpha_1,\dots,\alpha_{n} \in k$ satisfy $f_i(\alpha_1,\dots,\alpha_{n})=0$ for all $i$, then $\alpha_1=\dots=\alpha_{n}=0$.  This will imply that $\supp A/I_c = \{(0, \ldots, 0)\}$ and hence $A/I_c$ has dimension $0$, completing the proof.

Suppose that the $\alpha_i$ take the distinct values $\{s_1,\dots,s_{r}\}$, where $s_i$ occurs with multiplicity $m_i>0$ so that $g(z)=\prod_i (1-s_iz)^{m_i}$.  In these terms, we have that 
\[
...
\begin{remark}
In the proof of Proposition \ref{prop:ci}, we require that $c$ avoids $\{-1, 1,\dots,(p-1)/2\}$ for Proposition \ref{prop:linind} and that $c$ avoids $\{\frac{l+r}{\sum_i m_i}\}$ for each of the finitely many applications of Lemma~\ref{lem:resbyparts}.  These are the only uses of the assumption that $c$ is generic, so Proposition \ref{prop:ci} and Theorem \ref{thm:main} hold for $c$ avoiding this finite set of values.
\end{remark}
