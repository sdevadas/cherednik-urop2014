...
In the general case of an ideal $I$ in a polynomial ring $k[X_1,\dots,X_n]$, $k[X_1,\dots,X_m]/I$ is a complete intersection if $I$ is generated by $l$ elements with $\dim k[X_1,\dots,X_n]/I=m-l$. 
...
\end{proposition}
\begin{proof}
By Proposition~\ref{prop:linind}, $I_c$ has $n-1$ linearly independent generators $\{f_i\}$. Hence we wish to show that $A/I_c$ has dimension $0$, for which it suffices to show that if $\alpha_1,\dots,\alpha_{n} \in k$ satisfy $f_i(\alpha_1,\dots,\alpha_{n})=0$ for all $i$, then $\alpha_1=\dots=\alpha_{n}=0$.  
